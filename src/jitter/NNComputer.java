...
public class NNComputer
{
	...
	public NNComputer(Vector3f[] v, float minDist)
	{
		this.v = v;
		this.minDist = minDist;
	}

	public NNComputer(Vector3f[] v)
	{
		this.v = v;
	}
				...
				for (int y_i = y - 1; y_i < y + 2; y_i++)
				{
					...
					for (int z_i = z - 1; z_i < z + 2; z_i++)
					{
						...
						for (int j : grid[x_i][y_i][z_i])
						{
							...
							//										num++;
							float d = Vector3fUtil.dist(v[i], v[j]);
							if (d < minD)
							{
								minD = d;
								minIdx = j;
	...
	public static void main(String[] args)
	{
		...
		for (int i = 0; i < v.length; i++)
		{
			v[i] = Vector3fUtil.randomVector(1.0f, r);
			//System.out.println(Vector3fUtil.toNiceString(v[i]));
		}
		StopWatchUtil.start("naive");
		NNComputer nn1 = new NNComputer(v);
		nn1.computeNaive();
		StopWatchUtil.stop("naive");
		StopWatchUtil.start("fast");
		NNComputer nn2 = new NNComputer(v, 0.15f);
		nn2.computeFast();
		StopWatchUtil.stop("fast");
		System.out.println(ArrayUtil.toString(nn1.getNeigbohrs()));
		System.out.println(ArrayUtil.toString(nn2.getNeigbohrs()));
		StopWatchUtil.print();
	}
}
