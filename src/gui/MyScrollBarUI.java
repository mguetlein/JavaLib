...
public class MyScrollBarUI extends ScrollBarUI implements LayoutManager, SwingConstants
{
	//	public static Color TRANSLUCENT = new Color(0, 0, 0, 0);
	public static Color BUTTON = Color.BLUE;
	public static Color TRACK = Color.CYAN;
	public static Color BACKGROUND = Color.BLACK;
	public static Color BORDER = Color.MAGENTA;
	...
	protected void configureScrollBarColors()
	{
		...
		thumbHighlightColor = BORDER;// UIManager.getColor("ScrollBar.thumbHighlight");
		thumbLightShadowColor = BORDER.darker();//.getColor("ScrollBar.thumbShadow");
		thumbDarkShadowColor = BORDER.darker().darker();// UIManager.getColor("ScrollBar.thumbDarkShadow");
		thumbColor = BUTTON;// UIManager.getColor("ScrollBar.thumb");

		trackColor = TRACK;
		trackHighlightColor = TRACK;// UIManager.getColor("ScrollBar.trackHighlight");
	}
	...
	{
		LazyActionMap.installLazyActionMap(scrollbar, MyScrollBarUI.class, "ScrollBar.actionMap");
	...
	{
		return (scrollbar.getOrientation() == JScrollBar.VERTICAL) ? new Dimension(scrollBarWidth, 48) : new Dimension(
				48, scrollBarWidth);
	}
	...
	{
		return new MyArrowButton(orientation, BUTTON, thumbLightShadowColor, thumbDarkShadowColor, thumbHighlightColor);
	}
	...
	private void updateButtonDirections()
	{
		...
		if (scrollbar.getComponentOrientation().isLeftToRight())
		{
			if (incrButton instanceof MyArrowButton)
			{
				((MyArrowButton) incrButton).setDirection(orient == HORIZONTAL ? EAST : SOUTH);
			}
			if (decrButton instanceof MyArrowButton)
			{
				((MyArrowButton) decrButton).setDirection(orient == HORIZONTAL ? WEST : NORTH);
			}
		}
		else
		{
			if (incrButton instanceof MyArrowButton)
			{
				((MyArrowButton) incrButton).setDirection(orient == HORIZONTAL ? WEST : SOUTH);
			}
			if (decrButton instanceof MyArrowButton)
			{
				((MyArrowButton) decrButton).setDirection(orient == HORIZONTAL ? EAST : NORTH);
			}
		}
	}
	...
	private static class Actions extends UIAction
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			{
				scroll(scrollBar, MyScrollBarUI.MIN_SCROLL, true);
			}
			else if (key == MAX_SCROLL)
			{
				scroll(scrollBar, MyScrollBarUI.MAX_SCROLL, true);
			}
		}
		...
		private void scroll(JScrollBar scrollBar, int dir, boolean block)
		{
			...
			else if (dir == MyScrollBarUI.MIN_SCROLL)
			{
				scrollBar.setValue(scrollBar.getMinimum());
			}
			else if (dir == MyScrollBarUI.MAX_SCROLL)
			{
	...
	public static void main(String args[])
	{
		...
		System.out.println(MyScrollBarUI.class.getName());
		UIManager.put("ScrollBarUI", MyScrollBarUI.class.getName());

		JPanel p = new JPanel();
		p.setBackground(Color.LIGHT_GRAY);
		p.setPreferredSize(new Dimension(1200, 900));
		JScrollPane s = new JScrollPane(p);
		s.getViewport().setBackground(Color.ORANGE);
		s.setBackground(Color.YELLOW);
		//		((MyScrollBarUI) s.getVerticalScrollBar().getUI()).configureScrollBarColors();

		UIManager.put("Panel.background", new ColorUIResource(Color.WHITE));
		SwingUtil.showInDialog(s, new Dimension(400, 300));
		System.exit(0);
	}
}
